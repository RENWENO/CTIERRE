...
class TextCnnModel(nn.Module):
    ...
    def forward(self, x):
        ...
        #unsqueeze(1) means operating in the first dimension, raising the dimension
...
class BertTextModel_encode_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Take the vectors encoded from each layer
        hidden_states = outputs.hidden_states  # 13 * [batch_size, seq_len, hidden] The first layer is not required on the embedding layer
        cls_embeddings = hidden_states[1][:, 0, :].unsqueeze(1)  # [batch_size, 1, hidden]
        # The first token(cls vector) of each layer is extracted and pieced together as input to the textCnn
        for i in range(2, 13):

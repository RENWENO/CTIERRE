...
class TextCnnModel(nn.Module):
    def __init__(self):
        ...
        #parsers().num_filters卷积的输出元数2，parsers().filter_sizes卷积核大小[2,3,4]
        #num_filter_total表示
        ...
        #nn.Parameter()表示module的可训练参数
        self.bias = nn.Parameter(torch.ones([parsers().class_num]))
        #这里的卷积层分别时nn.Conv2d(1,2,kernel_size=(2,765))
        #这里的卷积层分别时nn.Conv2d(1,2,kernel_size=(3,765))
        #这里的卷积层分别时nn.Conv2d(1,2,kernel_size=(4,765))
    ...
    def forward(self, x):
        ...
        #unsqueeze(1)表示在第1个维度上进行操作，升维操作
